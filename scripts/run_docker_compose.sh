...
echo -e "\nStop running Docker containers with container name price-change-writer, and remove them...\n"
docker stop $(docker ps -a | grep price-change-writer | awk '{print $1}')
docker rm $(docker ps -a | grep price-change-writer | awk '{print $1}')
